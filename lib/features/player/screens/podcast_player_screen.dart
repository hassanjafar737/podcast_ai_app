...
class _PodcastPlayerScreenState extends State<PodcastPlayerScreen> {
  ...
  Future<void> _calculateTotalDuration() async {
    ...
    try {
      ...
      debugPrint("Error calculating absolute multi-track positions: $e");
    }
  ...
  Duration _getAbsolutePosition(PodcastProvider provider) {
    int currentIndex = provider.player.currentIndex ?? 0;
    Duration acc = Duration.zero;
    for (int i = 0; i < currentIndex && i < _trackDurations.length; i++) {
      acc += _trackDurations[i];
  ...
  void _seekAbsolute(Duration absPos, PodcastProvider provider) {
    Duration acc = Duration.zero;
    for (int i = 0; i < _trackDurations.length; i++) {
      Duration trackD = _trackDurations[i];
      if (absPos >= acc && absPos <= acc + trackD) {
  ...
  Widget build(BuildContext context) {
    ...
    // Fix critical crash calculation bounds by targeting absolute total metadata limit
    final remainingDuration = _totalDuration - currentAbsPos;
            ...
            Container(
              ...
              child: Row(
                children: [
                  GestureDetector(
                    onTap: () => Navigator.pop(context),
                    child: Icon(Icons.keyboard_arrow_down_rounded, color: Colors.white70, size: 26.sp.clamp(20.0, 32.0)),
                  ),
                  SizedBox(width: 8.w),
                  Text(
                    "VOX AI",
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      width: double.infinity,
                      height: 250.h.clamp(240.0, 380.0),
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(28.r),
                        image: const DecorationImage(
                          image: AssetImage("assets/images/profile1.png"),
                    ...
                    Align(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          color: Colors.white,
                          fontWeight: FontWeight.bold,
                          fontSize: 26.sp.clamp(22.0, 34.0),
                          height: 1.15,
                        ),
                      ),
                    ),
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text(_formatDuration(currentAbsPos), style: TextStyle(color: Colors.white38, fontSize: 11.sp.clamp(9.0, 13.0), fontWeight: FontWeight.w600)),
                                Text("-${_formatDuration(remainingDuration)}", style: TextStyle(color: Colors.white38, fontSize: 11.sp.clamp(9.0, 13.0), fontWeight: FontWeight.w600)),
                              ],
                            ),
                          ),
                        ],
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          icon: Icon(Icons.replay_10_rounded, color: Colors.white70, size: 28.sp.clamp(24.0, 32.0)),
                          onPressed: () => provider.player.seek(provider.position - const Duration(seconds: 10)),
                        ),
                        GestureDetector(
                          onTap: () => provider.togglePlay(),
                          child: Container(
                            width: 75.w.clamp(60.0, 95.0),
                            height: 75.w.clamp(60.0, 95.0),
                            decoration: BoxDecoration(
                              shape: BoxShape.circle,
                              gradient: const LinearGradient(
                                colors: [Color(0xff2563EB), Color(0xff8B5CF6)],
                        ...
                        IconButton(
                          icon: Icon(Icons.forward_10_rounded, color: Colors.white70, size: 28.sp.clamp(24.0, 32.0)),
                          onPressed: () => provider.player.seek(provider.position + const Duration(seconds: 10)),
                        ),
                        Icon(Icons.playlist_add_rounded, color: Colors.white70, size: 24.sp.clamp(20.0, 28.0)),
                      ],
                    ),
                    SizedBox(height: 30.h),
                    ...
                    Row(
                      ...
                      children: [
                        PlayerActionButton(icon: Icons.share_outlined, title: "SHARE", onTap: () {}),
                        PlayerActionButton(icon: Icons.file_download_outlined, title: "OFFLINE", onTap: () {}),
                        PlayerActionButton(icon: Icons.favorite_border_rounded, title: "LIKE", onTap: () {}),
                        PlayerActionButton(icon: Icons.timer_outlined, title: "SLEEP", onTap: () {}),
                      ],
                    ),
